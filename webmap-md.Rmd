...
This R markdown file details the workflow of the design process for the web application (https://gds-funtimes.shinyapps.io/webmap/). Firstly, Shiny provides the overarching web infrastructure for projecting the spatio-temporal geography of crime densities across Liverpool. Shiny uses a single-page application architecture which, in turn, uses AJAX for creating fluid user experiences without the need for constant page reloads. In our case, this is important because every page reload also pulls a 12 month window of crime occurences client-side from the Police.uk API - given a web server, an alternative would be a server-side cache of monthly crime data.

As Shiny uses a reactive programming model, the user is able to interact through the browser interface to explore how clusters of crime occurences fluctuate over time. This is achieved by dragging the month slider on the data controller to the selected date range. Doing so changes the 

By either clicking or zooming on the clusters, finer granularity can be observed until individual markers associated with each cluster enter the browser's viewport. Individual markers are clickable and contain popup information pertaining the 


On the front-end, the <i>flatly</i> theme from the shinythemes package is used to style the application. Yet, further customisation is introduced with a custom CSS script to fine-tune the aesthetical properties of the dashboard in the user's viewport.
