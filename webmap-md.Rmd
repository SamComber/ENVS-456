...
The resulting Shiny application displays several interesting patterns relating to the spatio-temporal distribution of crimes in Liverpool. In terms of spatial patterning, we observe the largest cluster of crimes in the city centre area stretching from Canning Dock to Mount Pleasent. Outside of this, we observe clusters of high crime incidents in the North-East metropolitan triangle of Fazakerley, Croxteth and West Derby. A third cluster can also be observed constituting the areas of Kensington, Wavertree and Old Swan. By contrast, we observe the lowest clustering of crimes in the South-East of Liverpool in suburban areas such as Halewood, Woolton and Gateacre. Our density-based DBSCAN plot confirms these observations, with a larger cluster of crimes obsrved around the city centre which expands as one increases the sample size using the slider. Similarly, at 45,000 crimes, we can observe the distinct crime cluster of Kensington, Wavertree and Old Swan in the centre of the plot. When we apply the time dimension by toggling the month slider, we subset the data and are able to explore how the spatial clustering of crime changes temporally. To complement this, the loess line of the time-series plot shows a gradual increase in the number of crime incidents from 2015-12 to 2016-11. 

Finally, the bar chart in the bottom-right column displays crime frequencies by category. Overall, we observe anti social behaviour as the most prolific street-level crime followed by violent crime. These rankings do not seem to fluctuate much when toggling the sample size slider. An interesting extension might be to allow further interactivity by permitting a user to reactively filter crimes displayed by category using the HTML input checkbox object.

<!-- 

1) frequency of types of crime in bar chart
2) time series of crimes increasing including loess line to capture overall trend
3) dbscan density-based clustering 
4) marker cluster observes patterning of crime in city centre
...
server <- function(input, output, session) {
  ...
  master <- data.frame(id=numeric(0), category=character(0), lat=character(0), lon=character(0),
                       month=character(0), outcome_status=character(0))
  
  # note: we choose not to grow the dataframe in a for loop due to computational cost
  ...
  document$category <- paste(toupper(substring(document[,c("category")], 1, 1)),
                             substring(document[,c("category")], 2), sep="")
  ...
  output$crimetable <- DT::renderDataTable({
    # subset crimes table
    new.document <- data.frame(document$category, document$lat, document$lon, document$name,
                               document$outcome_status ,document$month)
    # rename columns
